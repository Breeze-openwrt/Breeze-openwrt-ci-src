...
### 2. 🧲 架设源文件高速缓存网 (`actions/cache`)
**原由**：最耗时的过程之一，就是使用 `make download -j8` 全球拉取几十数百个包的 C 语言源码。
**高阶动作**：我们将植入 GitHub 的护城河插件 `actions/cache@v3`。通过读取你源码的指纹，如果我们昨天刚编译过同样的底层依赖，今天它将直接一秒内从旁边的服务器抽调几百兆的源码包（`dl/` 目录）到你的肉机上，跳过缓慢的网络发包。

### 3. 多线程极致规划
**小提示**：目前脚本默认运用了 `make -j$(nproc)`，这是理论上的极速火力满载，本身无需改变。我们将为其添加保姆级解说。
